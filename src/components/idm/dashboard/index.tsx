...
import { formatCurrency, formatCurrencyShort, clubToString, parseWitaDate, toStrictDivision } from '@/lib/utils';
          ...
          <div className="flex-1 flex flex-col p-4 sm:p-6 lg:p-8 min-w-0">
            ...
            <div className="mt-auto pt-3 sm:pt-4">
              ...
              <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 lg:gap-3 mt-1.5 sm:mt-2">
                {t?.scheduledAt && <span className="inline-flex items-center gap-1 px-1.5 py-0.5 sm:px-0 sm:py-0 rounded sm:rounded-none bg-black/30 sm:bg-transparent text-[9px] sm:text-xs lg:text-sm text-white/80"><Calendar className={`w-2.5 h-2.5 sm:w-3 sm:h-3 lg:w-4 lg:h-4 ${dt.neonText}`} />{parseWitaDate(t.scheduledAt)?.toLocaleDateString('id-ID', { weekday: 'short', day: 'numeric', month: 'short' })}</span>}
                {t?.scheduledAt && <span className="inline-flex items-center gap-1 px-1.5 py-0.5 sm:px-0 sm:py-0 rounded sm:rounded-none bg-black/30 sm:bg-transparent text-[9px] sm:text-xs lg:text-sm text-white/80"><Clock className={`w-2.5 h-2.5 sm:w-3 sm:h-3 lg:w-4 lg:h-4 ${dt.neonText}`} />{parseWitaDate(t.scheduledAt)?.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}</span>}
