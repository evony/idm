...
import { formatCurrency, parseWitaDate } from '@/lib/utils';
...
export function TournamentManager({ division, dt, stats, setConfirmDialog }: TournamentManagerProps) {
  ...
  const openEditDialog = (t: { id: string; name: string; weekNumber: number; format: string; defaultMatchFormat: string; prizePool: number; bpm?: number | null; location?: string | null; scheduledAt?: string | null }) => {
    setEditForm({
      ...
      bpm: String(t.bpm ?? ''),
      location: t.location || '',
      scheduledAt: t.scheduledAt ? (() => { const d = parseWitaDate(t.scheduledAt); return d ? `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}T${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}` : ''; })() : '',
    });
                ...
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                      ...
                      {t.scheduledAt && <span className="text-[10px] text-muted-foreground flex items-center gap-0.5"><Calendar className="w-2.5 h-2.5" />{parseWitaDate(t.scheduledAt)?.toLocaleDateString('id-ID', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}</span>}
                      {t._count && <>
