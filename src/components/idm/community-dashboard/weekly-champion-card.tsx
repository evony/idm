...
import { Crown, Trophy, Medal, Zap, Flame, Music, Shield, Star, Wallet, Users } from 'lucide-react';
...
function DivisionChampionCard({
  ...
  /* ─── Filled state ─── */
  // Reorder players for podium: [2nd, 1st, 3rd]
  const podiumOrder = championPlayers.length >= 3
    ? [championPlayers[1], championPlayers[0], championPlayers[2]]
    : championPlayers.length === 2
      ? [championPlayers[1], championPlayers[0]]
      : championPlayers;

  const podiumRanks = championPlayers.length >= 3
    ? [2, 1, 3]
    : championPlayers.length === 2
      ? [2, 1]
      : [1];
            ...
            {podiumOrder.map((player, podiumIdx) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="relative w-full">
                    ...
                    >
                      ...
                      <div className="absolute top-2 left-1/2 -translate-x-1/2 z-10">
                        <span
                          className={`${rankBadgeClass} inline-flex items-center justify-center px-2 py-0.5 rounded-md text-[9px] font-black leading-none tracking-wider`}
                        >
                  ...
                  {/* MVP label below avatar for center player */}
                  {isMvp && isCenter && (
                    <Badge
