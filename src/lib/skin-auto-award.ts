...
// - Champion skin: champion_1/2/3 — semua juara Royal Gold (satu kesatuan)
...
/** Map team rank (1/2/3) to skin type — semua Royal Gold */
function getChampionSkinType(rank: number): string {
  if (rank === 1) return 'champion_1';
  if (rank === 2) return 'champion_2';
  if (rank === 3) return 'champion_3';
  return 'champion'; // fallback for unknown rank
}
...
export async function autoAwardTournamentSkins(
  ...
  // ===== AWARD CHAMPION SKINS TO PODIUM TEAM MEMBERS =====
  for (const { teamId, rank } of teamRankings) {
    if (rank < 1 || rank > 3) continue; // Only rank 1-3 get skins
