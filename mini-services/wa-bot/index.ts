...
// Load from THIS script's directory, not CWD (fixes running from project root)
// ============================================
...
try {
  // Priority 1: .env in script's own directory (mini-services/wa-bot/.env)
  // This ALWAYS takes precedence for DATABASE_URL to avoid using the root project's SQLite DB
  const envPath = join(__dirname, '.env');
  if (existsSync(envPath)) {
    const envContent = readFileSync(envPath, 'utf-8');
    const waBotEnv: Record<string, string> = {};
    ...
    // Always override DATABASE_URL and DIRECT_DATABASE_URL from our own .env
    // (prevents using root project's SQLite URL)
    for (const [key, val] of Object.entries(waBotEnv)) {
      if (key === 'DATABASE_URL' || key === 'DIRECT_DATABASE_URL' || !process.env[key]) {
        process.env[key] = val;
      }
    }
    console.log(`  📄 .env file loaded from: ${envPath}`);
  } else {
    // Priority 2: Fallback to CWD .env
    ...
    if (existsSync(fallbackPath)) {
      ...
      console.log(`  📄 .env file loaded from CWD fallback: ${fallbackPath}`);
    }
  }
} catch (_) { /* ignore */ }
