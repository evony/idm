 ...
 * The loader injects f_auto,q_auto:eco,w_{width},c_limit — we must preload the SAME
...
export default async function Home() {
  ...
  // High-priority: Champion player avatars (w=144 for the MVP-style card)
    ...
    if (stats?.allSeasons) {
      ...
      if (championSeason?.championPlayer?.avatar) {
        ...
        if (avatarUrl.startsWith('http')) {
          // ★ Preload the OPTIMIZED URL (w=144, f_auto, q_auto) — matches the Image component
          criticalPreloads.push({
            url: optimizeCloudinaryUrl(avatarUrl, 144),
            fetchPriority: 'high',
          });
        }
      }

      // ★ Also preload champion CLUB logo (w=112 for ClubChampionCard)
      if (championSeason?.championClub?.logo) {
        ...
        if (logoUrl.startsWith('http')) {
          criticalPreloads.push({
            url: optimizeCloudinaryUrl(logoUrl, 112),
            fetchPriority: 'high',
          });
        }
      }

      // ★ Preload club member avatars (w=64 for top performers row)
        ...
        for (const member of championSeason.championClub.members.slice(0, 5)) {
          if (member.avatar) {
            ...
            if (memberAvatarUrl.startsWith('http')) {
              criticalPreloads.push({
                url: optimizeCloudinaryUrl(memberAvatarUrl, 64),
  ...
  return (
    <>
      ...
      {/* ★ Preload season champion avatars & club logos — eliminates the visible delay
          ...
          URLs are already optimized (f_auto,q_auto:eco,w_XXX) to match the Cloudinary loader,
