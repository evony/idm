                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                        {(showAllClubs ? sortedClubs : sortedClubs.slice(0, 10)).map((club, idx) => {
                          ...
                          return (
                            ...
                            >
                              <div className={`club-card club-card-shimmer relative rounded-2xl bg-mid border text-center transition-all duration-300 overflow-hidden ${
                                isChampion ? 'border-[rgba(212,168,83,0.30)]' : 'border-[rgba(212,168,83,0.08)]'
                              }`}>
                                {/* ── Top accent line with gradient ── */}
                                <div className="absolute top-0 left-0 right-0 h-[2px] z-20" style={{
                                  background: isChampion
                                    ? 'linear-gradient(90deg, transparent 5%, #d4a853 30%, #f5d77a 50%, #d4a853 70%, transparent 95%)'
                                    : 'linear-gradient(90deg, transparent 10%, rgba(212,168,83,0.4) 40%, rgba(212,168,83,0.6) 50%, rgba(212,168,83,0.4) 60%, transparent 90%)'
                                }} />
